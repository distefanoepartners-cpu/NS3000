...
export async function POST(request: Request) {
  try {
    ...
    if (activeBookings && activeBookings.length > 0) {
      // LOGICA FASCE ORARIE
      // Nota: 'evening' (sera, dopo le 18) è INDIPENDENTE: non collide con
      // full_day/morning/afternoon (che terminano alle 18) e viceversa.
      ...
      const hasEvening = activeBookings.some((b: any) => b.time_slot === 'evening')

      // Se sto prenotando la SERA: collide solo con un'altra sera
      if (time_slot === 'evening') {
        if (hasEvening) {
          return NextResponse.json({ available: false, reason: `Sera già prenotata per questa data` })
        }
        return NextResponse.json({ available: true })
      }

      // Per full_day/morning/afternoon, la sera esistente NON è un conflitto → la ignoriamo.

      // Se c'è full day → occupato (per full_day/morning/afternoon)
      if (hasFullDay) {
        return NextResponse.json({
          available: false,
      ...
      // Se voglio prenotare full day ma c'è già mattina/pomeriggio → occupato
      // (la sera NON conta: full day finisce alle 18)
